...
if (pages_to_report == "DevProg" || pages_to_report == "all" & exists("ovary_map_dataset_plotted")) {
  ...
  ovary_map_legend_raw =
    figure_legends_table %>%
    filter(plot == "ovary_map") %>%
    filter(dataset == ovary_map_dataset_plotted) %>%
    pull(text)
  ovary_map_legend = str_replace(ovary_map_legend_raw, "input_variable", input$variable)
  fig_nums("ovary_map", ovary_map_legend)
  
}else{distPlot_to_display = FALSE}

if (pages_to_report == "heatmap" || pages_to_report == "all" & exists("heatmap_dataset_plotted")) {
  heatmap_to_display = TRUE
  
  heatmap_legend_raw =
    figure_legends_table %>%
    filter(plot == "heat_map") %>%
    filter(dataset == heatmap_dataset_plotted) %>%
    pull(text)
