...
if (input$violin_normalization_option == "each_gene") {
  TKV_normaliztion_text = " normalized to expression in UAS-tkv "
}else if (input$violin_normalization_option == "unNorm") {
  TKV_normaliztion_text = " "
}

if(input$SeqDataset == "Input_seq"){
  fig_nums("ovary_map", paste("Plot of", input$variable,"log~2~(TPM+1) over development."))
  ...
  if (input$violin_geneList_option == "GO_term_selection") {
    fig_nums("violin_plot", paste0("Plot of genes in the GO-term \"", input$GO_term,"\" over development representing log~2~(TPM+1)", TKV_normaliztion_text ,"of input mRNA"))
  }else if(input$violin_geneList_option == "Custom_selection"){
    fig_nums("violin_plot", paste0("Plot of log~2~(TPM+1)", TKV_normaliztion_text, "of a custom gene group over development of input mRNA expression"))
  }
  ...
  if (input$violin_geneList_option == "GO_term_selection") {
      fig_nums("violin_plot", paste0("Plot of log~2~(TE)", TKV_normaliztion_text, "of genes in the GO-term \"", input$GO_term, "\" over development."))
  }else if(input$violin_geneList_option == "Custom_selection"){
    fig_nums("violin_plot", paste0("Plot of log~2~(TE)", TKV_normaliztion_text, "of a custom gene group over development."))
  }
}
```
<br><br>
