...
GO_plot_from_panther = function(all_GO_file_names){
  ...
  GO_file_read_and_clean = function(GO_file_name){
    ...
    GO_title = str_replace_all(GO_file_comparision, "_", " ")
    ...
    GO_file_cleaning$comparison = str_wrap(GO_title, width = 14)
    GO_file_cleaning$labels = str_wrap(GO_file_cleaning$GO_term, width = 50)
  ...
  top_cleaned_GOs_df = all_cleaned_GOs_df %>% arrange(-FDR) %>% group_by(comparison) %>% top_n(-7)
