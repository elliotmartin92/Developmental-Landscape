...
sc_seq_germline = read.xlsx("TPMs/SC_seq_expression_FBID.xlsx", sheet = 1)
germline_expressed_genes =
  sc_seq_germline %>% 
  filter(across("GSC/CB/2-cc":"St2") != 0) %>% 
  pull(FBGN)

allseq = allseq[rownames(allseq) %in% germline_expressed_genes,]
...
saveWorkbook(wb, file = "DE_Analysis/input_DE_pairwise_comparisions_germline.xlsx", overwrite = TRUE) #save workbook
...
write_rds(all_pairwise_comparisions_uniqueflat, file = "ShinyExpresionMap/developmentally_regulated_gene_list_germline.RDS")
